...
void MemoryConfig::update(size_t lastPoolSize, MallocMode lastMode, bool startFromCurrentSizeLevel) {
    size_t& sizeLevelIndex = (lastMode == DISCRETE ? fDiscreteSizeLevelIndex : fUnifiedSizeLevelIndex);
    size_t offset = (startFromCurrentSizeLevel ? sizeLevelIndex : 0);
    // Find the index of the first pool size that's greater than lastPoolSize
    std::vector<size_t>::iterator it = std::upper_bound(fSizeLevels.begin() + static_cast<std::vector<size_t>::difference_type>(offset), fSizeLevels.end(), lastPoolSize);
    sizeLevelIndex = (it == fSizeLevels.end() ? fSizeLevels.size() - 1 : static_cast<size_t>(std::distance(fSizeLevels.begin(), it)));
}
...
size_t MemoryConfig::roundUpPoolSizeWithLevel(size_t size, MallocMode mode) const {
    ...
    // Find the index of the first pool size that's greater than size
    std::vector<size_t>::const_iterator it = std::upper_bound(fSizeLevels.begin() + static_cast<std::vector<size_t>::difference_type>(sizeLevelIndex), fSizeLevels.end(), size);
    const size_t poolSize = (it == fSizeLevels.end() ? size : *it);
    return poolSize;
}
...
void MemoryPool::deallocateBlock(MemoryBlock* memoryBlock) {
    ...
    fAllocatedSize -= memoryBlock->size();
...
size_t MemoryManager::getTotalPoolSize() const {
    size_t bytes = 0;
...
size_t MemoryManager::getTotalNonEmptyPoolSize() const {
    size_t bytes = 0;
    for (PoolList::const_iterator iter = fDiscretePools.begin();
         iter != fDiscretePools.end(); ++iter) {
        MemoryPool* pool = *iter;
...
size_t MemoryManager::getTotalAllocatedBlockSize() const {
    size_t bytes = 0;
    for (PoolList::const_iterator iter = fDiscretePools.begin();
         iter != fDiscretePools.end(); ++iter) {
        MemoryPool* pool = *iter;
        bytes += pool->getAllocatedSize();
    }
    for (PoolList::const_iterator iter = fUnifiedPools.begin();
         iter != fUnifiedPools.end(); ++iter) {
        MemoryPool* pool = *iter;
...
void MemoryManager::freeEmptyPools() {
    size_t maxDiscretePoolSize = freeEmptyPoolsAndFindMaxPoolSize(fDiscretePools);
    size_t maxUnifiedPoolSize = freeEmptyPoolsAndFindMaxPoolSize(fUnifiedPools);
    fConfig.update(maxDiscretePoolSize, MallocMode::DISCRETE, /*startFromCurrentSizeLevel = */false);
    fConfig.update(maxUnifiedPoolSize, MallocMode::UNIFIED, /*startFromCurrentSizeLevel = */false);
}

size_t MemoryManager::freeEmptyPoolsAndFindMaxPoolSize(PoolList& poolList) {
    size_t maxPoolSize = 0;
    for (PoolList::iterator iter = poolList.begin(); iter != poolList.end();) {
        MemoryPool* pool = *iter;
        if (!pool->hasAllocatedBlocks()) {
            delete pool;
            iter = poolList.erase(iter);
        } else {
            // Find the max pool size that still has allocated blocks
